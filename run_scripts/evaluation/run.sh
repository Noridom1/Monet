...
# run_latent_policy.sh and validated again by the model adapter.
...
if [ ! -d "$MODEL_PATH" ]; then
  echo "[run-eval] ERROR: MODEL_PATH '$MODEL_PATH' not found. Run run_scripts/setup/local.sh first." >&2
  exit 1
fi
if [ ! -f "$EVAL_DIR/run_monet.py" ]; then
  echo "[run-eval] ERROR: VLMEvalKit not set up. Run run_scripts/evaluation/setup_kit.sh first." >&2
  exit 1
fi
...
# Refresh tracked Monet integration files on every run. setup_kit.sh is still
...
for d in $DATASETS; do
  python "$REPO_DIR/run_scripts/evaluation/eval_subset.py" --dataset "$d" --mode restore
  if [ "$SUBSET" = "head" ] || [ "$SUBSET" = "random" ]; then
    SIZE_ARGS=(); [ -n "$FRAC" ] && SIZE_ARGS+=(--frac "$FRAC"); [ -n "$N" ] && SIZE_ARGS+=(--n "$N")
    python "$REPO_DIR/run_scripts/evaluation/eval_subset.py" --dataset "$d" --mode "$SUBSET" --seed "$SEED" "${SIZE_ARGS[@]}"
  elif [ "$SUBSET" = "indices" ]; then
    python "$REPO_DIR/run_scripts/evaluation/eval_subset.py" --dataset "$d" --mode indices \
