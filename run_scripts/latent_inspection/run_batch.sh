...
# Prerequisite: bash run_scripts/latent_inspection/prepare_eval_samples.sh
set -euo pipefail
...
if [ ! -d "$MODEL_PATH" ]; then
  echo "[batch] ERROR: MODEL_PATH '$MODEL_PATH' not found. Run run_scripts/setup/local.sh first." >&2
  exit 1
fi
if [ ! -f "$MANIFEST" ]; then
  echo "[batch] ERROR: manifest '$MANIFEST' not found. Run prepare_eval_samples.sh first." >&2
  exit 1
fi
