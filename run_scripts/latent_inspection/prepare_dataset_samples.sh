#!/usr/bin/env bash
# Adapt a prepared dataset (run_scripts/evaluation/prepare_data.sh output) into an inspection
...
if [ ! -f "$DATA_DIR/samples.json" ]; then
  echo "[prepare-ds] ERROR: '$DATA_DIR/samples.json' not found. Run run_scripts/evaluation/prepare_data.sh first." >&2
  exit 1
fi
