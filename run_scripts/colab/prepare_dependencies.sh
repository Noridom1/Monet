...
usage() {
  cat <<'EOF'
Usage: bash run_scripts/colab/prepare_dependencies.sh [options]
...
uses run_scripts/00_setup_env.sh to create/update the pinned `monet` env.
EOF
}
...
OUTPUT="$(colab exec \
  -s "$SESSION" \
  -f "$REPO_DIR/run_scripts/colab/setup_inference_env.py" \
  --timeout "$TIMEOUT")"
printf '%s\n' "$OUTPUT"
