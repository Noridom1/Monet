...
usage() {
  cat <<'EOF'
Usage: bash run_scripts/setup/colab/download_outputs.sh --remote-dir PATH [options]
...
"$COLAB_PY" "$REPO_DIR/run_scripts/setup/colab/refresh_session_proxy.py" --session "$SESSION"
...
OUTPUT="$(colab exec \
  -s "$SESSION" \
  -f "$REPO_DIR/run_scripts/setup/colab/package_remote_directory.py" \
  --timeout "$TIMEOUT")"
printf '%s\n' "$OUTPUT"
