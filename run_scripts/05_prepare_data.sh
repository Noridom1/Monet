...
#   bash run_scripts/05_prepare_data.sh MyBench --repo org/MyBench --split test
#   bash run_scripts/05_prepare_data.sh VisualPuzzles --limit 20   # quick smoke test
set -euo pipefail
...
if [ "$#" -eq 0 ]; then
  echo "usage: bash run_scripts/05_prepare_data.sh <name> [--repo org/name] [--split S] [--config C] [--limit N]" >&2
  exit 1
fi

# For gated datasets, run `hf auth login` first.
python "$REPO_DIR/run_scripts/prepare_dataset.py" "$@"
